...
# get LedisDB
RUN wget -O ledisdb-src.tar.gz "https://github.com/freewill-fx/ledisdb/archive/v$LEDISDB_VERSION.tar.gz" && \
    tar -zxf ledisdb-src.tar.gz && \
    mkdir -p /go/src/github.com/freewill-fx/ && \
    mv ledisdb-$LEDISDB_VERSION /go/src/github.com/freewill-fx/ledisdb
...
RUN wget -O leveldb-src.tar.gz "https://github.com/google/leveldb/archive/$LEVELDB_VERSION.tar.gz" && \
    ...
    patch -p0 < /go/src/github.com/freewill-fx/ledisdb/tools/leveldb.patch && \
...
RUN export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/build/lib:/usr/lib && \
    ...
    mkdir -p /build/bin && \
    rm -rf /build/bin/* && \
    cd /go/src/github.com/freewill-fx/ledisdb && \
...
COPY --from=builder /go/src/github.com/freewill-fx/ledisdb/config/config-docker.toml /config.toml
